...
- **F1** · 🔥 **SEMANTICA lui `is_salariat` — SUB-DECLARARE.** Flagul însemna *„angajat cu normă întreagă altundeva"* — **un proxy, nu testul legal**. Art. 174 alin. (7) lit. a) cere **salarii de cel puțin 6 salarii minime pe an** (24.300 lei); lit. c), pensiile, **n-au prag deloc**.
   Un PFA salariat **part-time** sau angajat **la mijlocul anului** bifa „normă întreagă" fără să atingă pragul → codul îl scutea de podea → **declara mai puțin decât datora**. **Direcția rea:** sub-declararea nu-l lasă pe user cu bani în minus, îl lasă cu o datorie pe care i-am creat-o noi.
   ⚠️ **RĂDĂCINA NU E ÎN `contributii.py`.** Reconul a arătat că structura era deja corectă: coloanele `is_pensionar` / `is_salariat` sunt **separate** (`models.py:75-76`), iar OR-ul din `d212_calc.py:232` e **corect odată ce intrările sunt corecte**. Greșită era **întrebarea pusă userului**, deci înțelesul flagului — nu motorul. **Reparație de SEMANTICĂ, zero schemă, zero câmp nou.**
   ✅ **REZOLVAT** (PR #128): textele de pe ambele suprafețe poartă acum pragul, iar un gardian cade dacă diverg. *(Se închide la merge.)*
- **F2** · 🔥 **FLAGURILE DE ASIGURARE N-AU AN.** `is_salariat` și `is_pensionar` sunt **booleeni pe `User`**, fără dimensiune temporală — dar **D212 se depune pe un an anume**, iar statutul se schimbă de la an la an. Un flag setat în 2025 se aplică **tăcut** la calculul pe 2026.
   Botul întreabă **per-calcul** („în acel an", `declaratie_unica_ui.py:165`) și e **mai corect decât valoarea persistată** — dar **aruncă răspunsul**, nu-l scrie nicăieri. Deci suprafața corectă e cea care nu ține minte, iar cea care ține minte e cea imprecisă.
   **Cere DECIZIE DE SCHEMĂ:** persistăm per an (tabel/coloană pe an), sau persistăm un implicit și **confirmăm la fiecare calcul**?
   **Alin. (8)** — categoriile de la art. 154 din anul precedent + opțiunea art. 180 — **nu e modelat deloc**, și intră tot aici: e prin definiție o întrebare despre *anul precedent*, deci fără dimensiune temporală n-are unde trăi.
