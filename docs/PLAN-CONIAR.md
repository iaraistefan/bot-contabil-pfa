...
- **[august 2026] GENERATOR D212 XML — construit ȘI dovedit pe formularul real (PR #134 + #135, main `a60469a`, 1159 verzi).** Prima declarație pentru care avem **traseul verificat cap-coadă pe formularul ANAF**, nu doar cod validat contra unei scheme.
  **GOLUL PE CARE ÎL UMPLE:** din 2026 ANAF precompletează D212 în SPV, dar **ghidul oficial spune explicit** că precompletarea nu acoperă „veniturile din activități independente". Pentru un PFA plătit de platforme nerezidente (Bolt/Uber nu depun D205 în România) precompletarea vine **goală exact pe partea grea** — aia pe care noi o avem calculată din bonuri și extrase.
  **TRASEUL, dovedit:** formularul web ANAF (OpANAF 2736/2025, `anaf.ro/declaratii/duf`) a înlocuit PDF-ul inteligent. Are „Importă fișier salvat" cu `accept:".xml"`. Deci: generăm XML → userul îl importă → verifică → „Generează fișier PDF pentru depunere" → depune prin SPV. ⚠️ **DUKIntegrator NU poate valida acest XML** — ultimul plugin D212 publicat e J13.0.1 / 01.08.2025, pentru formularul VECHI (OpANAF 1929/2025); verificat pe kitul instalat local, `C:\dist\config\versiuniCurente.txt`.
  **TREI PROBE PE FORMULARUL PUBLIC (11-12 august 2026), rulate de Stefan.** Mediu Public, date fictive — întrebarea era o proprietate a formularului, nu a sesiunii, deci n-a fost nevoie de SPV și de date reale.
  **(1) Acceptă XML construit extern.** Antetul s-a populat, „Declarație inițială", rândul „(#1) 1016 — Activități independente în Sistem Real". **Și RECALCULEAZĂ, nu doar afișează:** pe net 80.000 a dat CASS 8.000 și impozit 7.200 — deși fișierul **nu conținea niciunul din cele două numere**. CAS 0, fiindcă fișierul avea `bifa131=0`.
  **(2) Bifele sunt PORTANTE, iar `oblig_realizat` e IGNORAT.** Cu payload complet (`cas_datorat="12150"`, `real_impozit_datorat_ai="5535"`), formularul a dat tot **CAS 0 și impozit 6.750** — adică `75.000 − 0 − 7.500`, recalculat din `cap11` + bife. **Formularul nu corectează o bifă greșită și nu citește secțiunile noastre de calcul.**
  **(3) Lanțul e IDENTIC cu al lui `d212_calc`.** După ce Stefan a tastat `48600` în „Baza anuală de calcul al CAS": impozit **6.750 → 5.535**, CAS **0 → 12.150**, CASS 7.500 neschimbat. Formularul a completat singur rd.3 și rd.5 și a dedus CAS din baza impozabilă. Verificat pe **două cazuri independente**.
  **CONSECINȚA CARE A SCHIMBAT PRODUSUL: baza CAS e singura piesă care NU se transferă.** E singurul număr din declarație care e o **alegere**, nu o derivare — formularul nu o poate calcula, doar o poate lăsa goală (și ANAF spune același lucru în ghidul de precompletare: *„Completați baza anuală de calcul (venitul ales)"*). Fără ea, declarația greșește **în ambele direcții deodată**: subdeclară CAS (−12.150) **și** supradeclară impozitul (6.750 în loc de 5.535, fiindcă CAS-ul nu se mai deduce). Ghidul spunea „baza CAS e o alegere, o poți mări" — informație utilă despre ceva ce **părea deja completat**. Acum e **pas obligatoriu**, cu cifra gata de tastat.
  **VERIFICAREA STĂ LA MOMENTUL ANGAJAMENTULUI, nu la începutul ghidului.** Ultima linie, lipită de butonul ireversibil: *„Înainte să apeși «Generează fișier PDF pentru depunere», uită-te la sumarul din dreapta. Dacă la CAS scrie 0, nu ai introdus baza — întoarce-te."* **Nu putem face greșeala imposibilă — nu controlăm formularul ANAF** — deci punem verificarea în singurul loc unde chiar funcționează. Un test pinuiește că e **ultima linie**, nu doar prezentă. Când CAS e 0 legitim (net sub plafon), pasul și avertismentul dispar amândouă: un „dacă vezi 0, întoarce-te" ar fi fals alarm.
  **`oblig_realizat` = REQUIRED-BY-SCHEMA / IGNORED-BY-FORM**, ambele jumătăți măsurate, documentat în modul ca să nu-l taie nimeni „ca optimizare" și să lovească în peretele Schematron după ce a scris codul. Fără element, cu aceleași bife: **11 eșecuri** (BR-D212-0041/0042/0043/0045/0046/0048/0049/0052/0053 pe CAS, 0054/0055 pe CASS). Consecința: valorile trebuie să fie coerente **între ele**, nu citite de cineva.
  **VALIDARE PE SCHEMELE OFICIALE, vendorizate cu SHA-256** (`app/integrations/anaf/scheme/d212/PROVENIENTA.md`, plus `.gitattributes -text` ca git să nu le strice LF→CRLF și să invalideze amprentele): XSD prin lxml + Schematron prin SaxonC (regulile sunt `queryBinding="xslt2"`, lxml e XSLT 1.0).
  **PETICUL AXEI DE ATRIBUTE — fără el validarea era teatru.** Skeleton-ul ISO Schematron traversează doar axa elementelor (`select="*"`), dar regulile ANAF sunt scrise pe `//@*`, fiindcă D212 ține **totul** în atribute. Rezultat: **6 reguli evaluate din 118**, cu „VALID" raportat vesel după 5% acoperire. Peticit la `select="*|@*"`; `PRAG_REGULI=60` cade dacă peticul dispare. **Verificat prin injectare**: cu petic 118, fără 6.
  **GARDIAN ARITMETIC PROPRIU — Schematron-ul ANAF NU verifică `rd.1 − rd.2`.** O **minciună coerentă** (net fals, recalculat la fel de fals peste tot) trece de toate cele 118 reguli. Dovedit ca test; dacă ANAF adaugă vreodată regula, testul cade și ștergem gardianul ca redundant.
  **`an_r` NU e anul veniturilor — e anul DEPUNERII.** Eticheta „Anul de raportare" induce în eroare: instrucțiunile ANAF folosesc **aceeași expresie** pentru anul veniturilor. Lanțul de dovezi: formularul are **două** casete de an (cap. I venituri / cap. II opțiune CASS) · XML-ul are **un singur** atribut · **BR-D212-0023** cere ca datele de activitate din cap. I să aibă anul `an_r − 1`. Deci anul veniturilor e implicit, fără câmp propriu. Lanțul e scris în trei locuri (modul, docstring de test, `PROVENIENTA.md`) ca următorul să nu-l refacă de la zero sau, mai rău, să presupună.
  **ANUL ACOPERIT SE CITEȘTE DIN `BR-D212-0006`, nu e hardcodat** — pachet nou de scheme pus în loc = adaptare fără o linie de cod. Verificarea de interval `2018..2100` a fost **scoasă**: promitea un domeniu pe care nu-l putea onora (pentru orice alt an produceam fișiere pe care propriul nostru Schematron le respingea). **Extragere eșuată → `RuntimeError`, fără valoare implicită** — un fallback tăcut ar fi refăcut exact problema reparată, doar că fără să se mai vadă.
  **BUG ÎN REGULA ANAF, ocolit:** BR-D212-0053 calculează `number(exists(@cas_retinut_platitor)) * number(@cas_retinut_platitor)` = `0 × NaN` = **NaN** când atributul lipsește, deși textul zice „dacă lipsește se consideră 0". De aceea scriem explicit `cas_retinut_platitor="0"` (și e adevărat: nimeni nu ne-a reținut CAS).
  **DOUĂ CÂMPURI N-AU PUTUT RĂMÂNE GOALE**, deși așa fuseseră planificate: nume/prenume (XSD `use="required"`, iar SN-D212-002 respinge atributele vide) și certificatul ONRC număr+dată (BR-D212-0095/0096). Sunt acum **parametri obligatorii** ai generatorului — le cerem apelantului, nu le ghicim spărgând `firma_nume`. Rămân goale, deliberat: inițiala tatălui, statutul contribuabilului, pierderile reportate.
  ⚠️ **NEDEPARTAJAT:** la proba (3) câmpul bazei CAS a fost editabil direct, fără nicio bifă de deblocat. Asta e compatibil și cu „`bifa_cas_real` a trecut prin import", și cu „câmpul nu e gated de nicio bifă". A doua e mai probabilă — `bifa_cas_real` **e un atribut din `oblig_realizat`**, iar proba (2) a arătat că elementul e ignorat. Consemnăm observația, nu inferența.
  ⚠️ **RĂMAS:** generatorul **nu e apelat de nicăieri** (fără wiring, fără migrări) · testele de Schematron se **sar tăcut** fără `saxonche` (gardianul aritmetic și cel al anului sunt Python curat, nu se sar) · pasul final în **SPV cu date reale** nu e făcut, și e al userului, nu al codului.
