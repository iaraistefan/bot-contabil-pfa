...
**CONSECINȚA PENTRU PRODUS:** dacă trece, masa de șoferi trece pe mașină **în proprietate** → **AMORTIZAREA devine mecanismul central de deductibilitate**, nu un caz marginal. De aici prioritatea blocantului „MOTORUL DE AMORTIZARE" din §5 (azi F5, cere research înainte de build; gardianul de achiziție — ÎNCHIS, PR #123 — oprește doar tratarea greșită, nu ține loc de motor). Tot de aici și interdicția din §3.4 pe generatorul de contract de comodat.
...
> *Auditat în cod (august 2026).* **Trei** au ieșit deja din listă: **RCA/CASCO pe comodat** (`posting.py:150-158` întoarce 0% pe comodat, felia 5B, tip nedeclarat → 50 conservator, comparație case-insensitive) · **TVA — declanșarea** (PR #121, vezi jurnalul) · **GARDIAN ACHIZIȚIE VEHICUL** (PR #123, vezi jurnalul — factura de autoturism nu mai cade pe `other_expense` = `FULL`; poarta e o **întrebare declanșată de sumă**, nu un clasificator). **Ordinea de mai jos e ordinea de atacat**, și e ordonată pe *ce deblochează*, nu pe mărime: întâi **sub-declararea** — direcția care creează datorii (F1), apoi **golul de comunicare** din aceeași zonă, de rezolvat în aceeași trecere (F2), apoi clasa cu cea mai mare expunere, verificabilă într-o trecere (F3), apoi **gardianul** care oprește reapariția problemei (F4), apoi ce cere research înainte de build (F5), apoi ce nu e muncă de cod și merge în paralel (F6).

> 📋 **PASUL 1 AL AUDITULUI PROZEI FISCALE E FĂCUT** — inventarul complet trăiește în **`docs/INVENTAR-PROZA-FISCALA.md`**: **96 de afirmații fiscale în proză, în 15 fișiere**, fiecare cu fișier:linie, temei (DA/parțial/NU) și severitate. **64 din 96 n-au niciun temei legal lângă ele.** Doar UNA (plafonul TVA) poartă forma completă articol + act + data verificării. Blocantul inițial „auditul prozei fiscale" s-a **despicat în trei** (F2-F4 de mai jos), pe baza a ce a găsit inventarul; restul celor 96 sunt **amânate deliberat**, cu trimitere la inventar.

- **F1** · 🔥 **GRANULARITATEA `asigurat_salariat` — SUB-DECLARARE.** `contributii.py` folosește **un singur boolean** pentru salariați și pensionari. Art. 174 alin. (7) îi tratează **diferit**: lit. a) salarii cere nivel **≥ 6 SMB** (24.300 lei/an); lit. c) pensii, **fără prag**.
   Un PFA salariat **part-time** sau angajat **la mijlocul anului** NU se califică pentru scutire — dar codul îl scutește → **sare podeaua** → **declară mai puțin decât datorează**. **Direcția rea:** sub-declararea nu-l lasă pe user cu bani în minus, îl lasă cu o datorie pe care i-am creat-o noi.
   **Alin. (8)** (categoriile de la art. 154 din anul precedent, plus opțiunea art. 180) **nu e modelat deloc**.
   ⚠️ **Cere schimbare de INTERFAȚĂ, nu doar de valoare** — flagul trebuie să poarte și nivelul, nu doar existența asigurării. De asta e deasupra lui F2, deși F2 e mai vizibil.
- **F2** · 🔥 **CONTRADICȚIA CASS — bug activ, nu audit.** `contributii.py:29-31,213-215` spune că cine e **sub 6 SMB dar are altă asigurare** (salariat/pensionar) plătește 10% pe venitul net **real, fără urcare la podea**. `dashboard.html:1312` îi spune userului că **sub 6 salarii minime CASS se calculează pe baza minimă de 6 salarii**. **Rezultate diferite pentru același om** — și al doilea e cel pe care userul îl citește efectiv.
   **Nu pot fi ambele adevărate: una e greșită ACUM.** De asta nu e sarcină de audit, ci bug de reparat — și de asta cele două **se verifică împreună**, nu separat: răspunsul corect le rezolvă pe amândouă. Codul își semnalează singur incertitudinea (`contributii.py:213-215`: surse secundare convergente, NU text primar).
   **ABSOARBE fostul blocant „validează CASS pentru asigurații sub podea"** — era aceeași întrebare, văzută doar dintr-o parte. Rămâne o conversație cu un contabil CECCAR, nu un build; dar acum știm că are și o consecință vizibilă în UI, nu doar o notă de subsol în cod.
- **F3** · 🔺 **CELE 8 TERMENE DE DEPUNERE — clasa în care a căzut „10 zile".** Fiecare dintre cele 8 declarații își afirmă termenul (inventar #1, 8, 12, 17, 21, 25, 31, 34) și **niciunul n-are temei legal lângă el**. Sunt exact felul de dată la care legea se poate muta fără ca nimic din cod să semnaleze — cum s-a și întâmplat o dată.
...
- **F4** · **TEMEIUL OBLIGATORIU PENTRU PROZA NOUĂ — gardian, nu curățenie.** Modelul e `vat_plafon_msg.py`: articol + act modificator + **data verificării pe forma consolidată**, scrise în modul. Azi **una singură din 96** arată așa.
   ⚠️ **Gardianul cere temei VERIFICAT CU DATĂ, nu prezența unei referințe.** Distincția nu e teoretică — a fost **dovedită la F2**: plafonul CASS era atribuit legii greșite (141/2025 în loc de 239/2025), avea o referință scrisă lângă el, arăta a temei și era numărat „DA" în inventar. O referință fără dată de verificare e o afirmație despre lege, nu o dovadă.
