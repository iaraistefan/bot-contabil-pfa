...
### 1.7 Plată — ✅ DECIS (Stripe+Billing) · 🔨 Felia 1 + 4a + 4b FĂCUTE
...
- Felii rămase: 2c webhook (verifică semnătura `STRIPE_WEBHOOK_SECRET` → `client_reference_id`/metadata → `set_subscription` status='active'; SINGURA scriere în DB), 3 Oblio factură (cere DATE CONIAR furnizor — CUI/reg.com./sediu/IBAN, LIPSESC).
